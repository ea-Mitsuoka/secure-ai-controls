...
- Model Armor のネットワーク要件（2026-07-16確認）: リージョンエンドポイントを VPC 内から使う場合は PSC エンドポイントが必要（docs.cloud.google.com/model-armor/data-residency）。restricted service 対応は `gcloud access-context-manager supported-services describe modelarmor.googleapis.com` で**実機確認済み（2026-07-16、GA・restricted VIP 対応）**
...
| 12  | VPC-SC 境界の方針（2026-07-16、旧未決4の残り）                      | **単一境界に2プロジェクト（決定#8）をともに含める**。restricted services は**全6件を実機確認済み（2026-07-16、gcloud supported-services describe）**: `aiplatform`・`dlp`・`bigquery`・`logging`・`monitoring`・`modelarmor` — すべて GA・restricted VIP 対応につき境界に含める。Model Armor のリージョンエンドポイントを VPC 内から使う場合は PSC エンドポイントが必要（§3.5）                                                                                                                                                                                                                                                                                        | §3.4           |
...
4. **確定済み（2026-07-16）**: プロジェクト構成は2プロジェクト（決定記録 #8）、VPC-SC 境界方針は単一境界＋restricted service 集合（決定記録 #12。modelarmor 含む全6サービスの対応を同日実機確認済み）。
...
**未決事項はすべて確定した（2026-07-16）。②向けの実機確認3件も同日前倒しで完了**（結果は [phase2-prep.md](phase2-prep.md) §1）: modelarmor の VPC-SC 対応＝GA（決定 #12 に反映済み）、`vertexai.allowedModels` の初期値候補＝gemini-3.5-flash / gemini-2.5-pro（FR-7 の tfvars で確定）、provider issue #24092＝open（`gcloud` 補完を維持）。
